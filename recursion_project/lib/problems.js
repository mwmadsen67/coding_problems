...
function flatten(data) {
    if (!Array.isArray(data)) return [data]; // return data as an array if its not an array, going to concat it

    let results = [];
    for(let i = 0; i < data.length; i++) {
        results = results.concat(flatten(data[i]));
    }
...
function pathFinder(directories, targetFile) {
    ...
    for(let i = 0; i < dirArray.length; i++) {
        if (dirArray[i] === targetFile) {
            ...
            let result = pathFinder(directories[dirArray[i]], targetFile);
